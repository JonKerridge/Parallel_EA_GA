...
class QueensPopulation extends DataClass{
  ...
  int numberOfQueens       //length of an individual's chromosome
  ...
  int create(List d){
    if ( instance == instances) return normalTermination
      ...
      //TODO make sure that an empty individual is returned
        individuals << new QueensIndividual(numberOfQueens) // MUST be changed
...
//    println "X3: $sB\n$mB\n$eB\n$mSb\n${individuals[child].board}"
  ...
  int queensCrossover(int parent1,
                int parent2,
                int child1,
                int child2,
                Random rng) {
    // must ensure crossover points are not 0 as board[0] is not used
    int c1 = rng.nextInt(numberOfQueens) + 1
    int c2 = rng.nextInt(numberOfQueens) + 1
    //ensure c1 and c2 are different
    while ( c1 == c2) c2 = rng.nextInt(numberOfQueens) + 1
