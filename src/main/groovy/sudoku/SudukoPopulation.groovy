...
class SudukoPopulation extends DataClass{
  ...
  List <SudokuIndividual> population = []  //
  List fileLines = []
  ...
  int create(List d){
    if ( instance == instances) return normalTermination
      ...
      //TODO make sure that an empty individual is returned
        population[i] = new SudokuIndividual(numberOfGenes)
...
//      println "QSR2: $start, $end, $splitPoint"
  ...
  boolean convergence(){
    ...
    return (population[first].getFitness()  == 162)
  }
...
//      println "Swapped: $child1 into $worst new fitness = ${population[child1].getFitness()}"
    }
    else {
      population.swap(worst, child2)
//      println "Swapped: $child2 into $worst new fitness = ${population[child2].getFitness()}"
    }
    return completedOK
  }
...
//    println "Pre Board"
//    preBoard.each{ println "$it"}
//    println "\n Fixed Numbers"
//    fixed.each{ println "$it"}
    return completedOK
  }
