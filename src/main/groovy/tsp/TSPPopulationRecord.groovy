...
class TSPPopulationRecord extends DataClass{
  ...
  List  <TSPIndividualRecord> individuals = []  //
  ...
  int create(List d){
    if ( instance == instances) return normalTermination
      ...
      //TODO make sure that an empty individual is returned
        individuals << new TSPIndividualRecord(numberOfGenes)  // MUST be changed
...
//      println "QSR2: $start, $end, $splitPoint"
  ...
  boolean convergence(){
    ...
    if (currentFitness < minFitness) {
      ...
      for ( i in 0 ..< individuals[first].nodesVisited.size())
        minNodesVisited << individuals[first].nodesVisited[i]
    }
    minCount += 1
...
//    println "X3: $sB\n$mB\n$eB\n$mSb\n${individuals[child].route}"
  ...
  int crossoverDantzig42 (int best,
    ...
    List <Integer> sB = extractParts(1, c1, individuals[best].route)
    List <Integer> mB = extractParts(c1, c2, individuals[best].route)
    List <Integer> mSb = extractParts(c1, c2, individuals[secondBest].route)
    List <Integer> eB = extractParts(c2, numberOfGenes, individuals[best].route)
...
//    eB = individuals[secondBest].route.getAt(c2 ..< numberOfQueens)
    sB = extractParts(1, c1, individuals[secondBest].route)
    mB = extractParts(c1, c2, individuals[secondBest].route)
    mSb = extractParts(c1, c2, individuals[best].route)
    eB = extractParts(c2, numberOfGenes, individuals[secondBest].route)
  ...
  int combineChildren(int best,
    ...
    worstFit = individuals[worst].getFitness()
    worst2Fit = individuals[secondWorst].getFitness()
...
//        println"overwriting both: w < c1, 2w < c2"
        ...
        individuals.swap(secondWorst, child2)
      } else {
        individuals.swap(worst, child1)
//        println "overwritng w < c1"
      ...
      if (child1Fit < worst2Fit) {
        ...
        individuals.swap(secondWorst, child1)
